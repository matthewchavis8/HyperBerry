...
void Gic::cpuInit() {
    ...
    uint32_t vtr = *reg(Frame::Hv, GicReg::Hv::VTR);
    m_numLr = (vtr & 0x3F) + 1;

    // Enable Group 1 interrupts and split priority drop from deactivation
    *reg(Frame::Cpu, GicReg::Cpu::CTLR) = GicReg::Cpu::CTLR_GRPEN1 | GicReg::Cpu::CTLR_EOIMODE;

    // Unmask all interrupts priorities
    *reg(Frame::Cpu, GicReg::Cpu::PMR) = 0xFF;

    // Set binary point to 0 (no priority grouping)
    *reg(Frame::Cpu, GicReg::Cpu::BPR) = 0x0;

    // Set up VMCR
    uint32_t vmcr = GicReg::Hv::VMCR_EN0;
    vmcr |= (0xFF >> 3) << GicReg::Hv::VMCR_PMR_SHIFT;
    *reg(Frame::Hv, GicReg::Hv::VMCR) = vmcr;

    // Bring up vGic Hypervisor interface
    *reg(Frame::Hv, GicReg::Hv::HCR) = GicReg::Hv::HCR_EN;

    // Clear all List Register entries
    for (uint32_t i {}; i < m_numLr; i++) {
        *reg(Frame::Hv, GicReg::Hv::LR0 + i * 4) = 0;
    }

    *reg(Frame::Hv, GicReg::Hv::APR) = 0;
}
...
void Gic::cpuReset() {
    ...
    *reg(Frame::Hv, GicReg::Hv::APR) = 0;
    *reg(Frame::Hv, GicReg::Hv::VMCR) = 0;
}

void Gic::enableIrq(uint32_t id) {
    uint32_t regIdx = id / 32;
    uint32_t bitMsk = (1U << (id % 32));
    *reg(Frame::Dist, GicReg::Dist::ISENABLER + regIdx * 4) = bitMsk;
}

void Gic::disableIrq(uint32_t id) {
    uint32_t regIdx = id / 32;
    uint32_t bitMsk = (1U << (id % 32));
    *reg(Frame::Dist, GicReg::Dist::ICENABLER + regIdx * 4) = bitMsk;
}

Gic::IrqAck Gic::ackIrq() {
    uint32_t iar = *reg(Frame::Cpu, GicReg::Cpu::IAR);
    return IrqAck { iar, iar & 0x3FF };
}

void Gic::endIrq(IrqAck irq) {
    *reg(Frame::Cpu, GicReg::Cpu::EOIR) = irq.iar;
}

int Gic::injectIrq(uint32_t virtId, uint32_t id) {
    uint32_t elsr0 = *reg(Frame::Hv, GicReg::Hv::ELSR0);
    uint32_t elsr1 = *reg(Frame::Hv, GicReg::Hv::ELSR1);
    ...
    for (uint32_t i {}; i < m_numLr; i++) {
        ...
        uint32_t lrVal = *reg(Frame::Hv, GicReg::Hv::LR0 + i * 4);
    ...
    *reg(Frame::Hv, GicReg::Hv::LR0 + static_cast<uint32_t>(freeLr) * 4) = lr;
...
void Gic::enableMainIrq(bool isEnable) {
    ...
    uint32_t hcr = *reg(Frame::Hv, GicReg::Hv::HCR);
    isEnable ? hcr |= GicReg::Hv::HCR_UIE : hcr &= ~GicReg::Hv::HCR_UIE;
    *reg(Frame::Hv, GicReg::Hv::HCR) = hcr;
}

void Gic::setPriorityLevel(uint32_t id, uint8_t priority) {
    volatile uint8_t* priorityReg =
            reinterpret_cast<volatile uint8_t*>(gDistBase + GicReg::Dist::IPRIORITYR + id);
    *priorityReg = priority;
}

void Gic::init() {
    // Disable Distributor
    *reg(Frame::Dist, GicReg::Dist::CTLR) = 0;

    // Find out how many interrupt lines are supported
    uint32_t typer = *reg(Frame::Dist, GicReg::Dist::TYPER);
    uint32_t numOfIrqLines = (typer & 0x1F) + 1;

    // Configure all SPIs (Shared Peripheral Interrupts)
    for (uint32_t i {}; i < numOfIrqLines; i++) {
        *reg(Frame::Dist, GicReg::Dist::IGROUPR + i * 4) = 0xFFFFFFFF; // Group all Non-secure SPIs
        *reg(Frame::Dist, GicReg::Dist::ICENABLER + i * 4) = 0xFFFFFFFF; // Disable all SPIs
        *reg(Frame::Dist, GicReg::Dist::ICPENDR + i * 4) = 0xFFFFFFFF;   // Clear any pending SPIs
        *reg(Frame::Dist, GicReg::Dist::ICACTIVER + i * 4) = 0xFFFFFFFF; // Clear any active SPIs
    }
    ...
    for (uint32_t i { 32 }; i < numOfSpis; i += 4) {
        *reg(Frame::Dist, GicReg::Dist::ITARGETSR + i) = 0x01010101;
    }

    // All SPis are level triggered
    uint32_t numCfgRegs = numOfIrqLines * 2;
    for (uint32_t i { 2 }; i < numCfgRegs; i++) {
        *reg(Frame::Dist, GicReg::Dist::ICFGR + i * 4) = 0x00000000;
    }

    // Re-enable Distributor
    *reg(Frame::Dist, GicReg::Dist::CTLR) = GicReg::Dist::CTLR_GRPEN1;
